...
- [`code/java/k8s-with-springboot/gke-backend-demo`](./code/java/k8s-with-springboot/gke-backend-demo) — Spring Boot + Cloud SQL, deployed to GKE with plain manifests (no Helm): Workload Identity, Cloud SQL Auth Proxy sidecar (no DB password), Secret Manager CSI, GKE Ingress with `BackendConfig`/`FrontendConfig`/`ManagedCertificate`; see its [gke-specific-features.md](./code/java/k8s-with-springboot/gke-backend-demo/gke-specific-features.md) for what's unique to GKE vs. portable Kubernetes, or its [console-walkthrough.md](./code/java/k8s-with-springboot/gke-backend-demo/console-walkthrough.md) for a click-by-click, why-explained deploy from a blank GCP project
